...
# ── 0. 사전 헬스체크: stable 살아있는지 확인 ──────
echo ">>> stable 헬스체크..."
...
if [ "$STABLE_STATUS" != "healthy" ]; then
    echo "❌ stable 서버가 healthy하지 않음: $STABLE_STATUS"
    echo "배포 중단 - stable 서버 점검 필요"
    exit 1
fi
echo "✅ stable 정상 확인"

# ── 1. .env 업데이트 ──────────────────────────────
sed -i "s/CANARY_TAG=.*/CANARY_TAG=${CANARY_TAG}/" .env

# ── 2. canary pull & 재시작 ───────────────────────
docker compose pull spring-canary
docker compose up -d --no-deps spring-canary

# ── 3. canary 헬스체크 ────────────────────────────
echo ">>> canary 헬스체크..."
CANARY_STATUS=""
for i in $(seq 1 10); do
    ...
    if [ "$CANARY_STATUS" = "healthy" ]; then
        echo "✅ canary 정상 확인"
        break
    fi

    echo "대기중... ($i/10) 현재상태: $CANARY_STATUS"
    sleep 5
done

if [ "$CANARY_STATUS" != "healthy" ]; then
    echo "❌ canary 헬스체크 실패 - 롤백"
